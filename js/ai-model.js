...
class HydrologicalAI {
    ...
    calculateRisk(level) {
        if (level > 4.2) return { level: 'HIGH', label: 'สูง', class: 'text-red-400' };
        if (level > 3.5) return { level: 'MEDIUM', label: 'ปานกลาง', class: 'text-yellow-400' };
        return { level: 'LOW', label: 'ต่ำ', class: 'text-green-400' };
    }
